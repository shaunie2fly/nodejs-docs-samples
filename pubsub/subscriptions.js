...
function getSubscriptionMetadata (subscriptionName, callback) {
  ...
  subscription.getMetadata((err, metadata) => {
    ...
    console.log(`Push config: %j`, metadata.pushConfig);
    console.log(`Ack deadline (s): ${metadata.ackDeadlineSeconds}`);
    callback();
  });
}
...
function getSubscriptionPolicy (subscriptionName, callback) {
  ...
  subscription.iam.getPolicy((err, policy) => {
    ...
    console.log(`Policy for subscription: %j.`, policy);
    callback();
  });
}
...
function setSubscriptionPolicy (subscriptionName, callback) {
  ...
  subscription.iam.setPolicy(newPolicy, (err, updatedPolicy) => {
    ...
    console.log(`Updated policy for subscription: %j`, updatedPolicy);
    callback();
  });
}
...
cli
  ...
  .command(`getPolicy <subscriptionName>`, `Gets the IAM policy for a subscription.`, {}, (options) => {
    program.getSubscriptionPolicy(options.subscriptionName, makeHandler(false));
  })
  .command(`setPolicy <subscriptionName>`, `Sets the IAM policy for a subscription.`, {}, (options) => {
    program.setSubscriptionPolicy(options.subscriptionName, makeHandler(false));
  })
  .command(`testPermissions <subscriptionName>`, `Tests the permissions for a subscription.`, {}, (options) => {
  ...
  .example(`node $0 getPolicy greetings-worker-1`, `Gets the IAM policy for a subscription named "greetings-worker-1".`)
  .example(`node $0 setPolicy greetings-worker-1`, `Sets the IAM policy for a subscription named "greetings-worker-1".`)
  .example(`node $0 testPermissions greetings-worker-1`, `Tests the permissions for a subscription named "greetings-worker-1".`)
  .wrap(120)
  .recommendCommands()
