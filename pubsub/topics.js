...
function getTopicPolicy (topicName, callback) {
  ...
  topic.iam.getPolicy((err, policy) => {
    ...
    console.log(`Policy for topic: %j.`, policy);
    callback();
  });
}
...
function setTopicPolicy (topicName, callback) {
  ...
  topic.iam.setPolicy(newPolicy, (err, updatedPolicy) => {
    ...
    console.log(`Updated policy for topic: %j`, updatedPolicy);
    callback();
  });
}
...
cli
  ...
  .command(`getPolicy <topicName>`, `Gets the IAM policy for a topic.`, {}, (options) => {
    program.getTopicPolicy(options.topicName, makeHandler(false));
  })
  .command(`setPolicy <topicName>`, `Sets the IAM policy for a topic.`, {}, (options) => {
    program.setTopicPolicy(options.topicName, makeHandler(false));
  })
  .command(`testPermissions <topicName>`, `Tests the permissions for a topic.`, {}, (options) => {
  ...
  .example(`node $0 getPolicy greetings`, `Gets the IAM policy for a topic named "greetings".`)
  .example(`node $0 setPolicy greetings`, `Sets the IAM policy for a topic named "greetings".`)
  .example(`node $0 testPermissions greetings`, `Tests the permissions for a topic named "greetings".`)
  .wrap(120)
  .recommendCommands()
